...
class MainActivity : AppCompatActivity() {
    ...
    private fun initializePlayer() {
        try {
            ...
            e.printStackTrace()
            Toast.makeText(this, "خطأ في المشغل: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun toggleRecording() { try { if(isRecording) stopRecording() else startRecording() } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun saveAccounts() { try { val json = JSONArray(); accounts.forEach { acc -> val obj = JSONObject(); obj.put("url", acc.url); obj.put("username", acc.username); obj.put("password", acc.password); json.put(obj) }; prefs.edit().putString("accounts", json.toString()).apply() } catch (e: Exception) { e.printStackTrace() } }
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?) = try { when(keyCode){
    ...
    } } catch (e: Exception) { super.onKeyDown(keyCode, event) }

    private fun togglePlayPause() { try { if(isPlayerPlaying) player.pause() else player.play() } catch (e: Exception) { e.printStackTrace() } }
    private fun changeAspectRatio() { try { currentAspectRatio=(currentAspectRatio+1)%3; playerView.resizeMode=when(currentAspectRatio){0->androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_FIT;1->androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_FILL;else->androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_ZOOM}; Toast.makeText(this,"📐 ${arrayOf("ملائم","ملء","تكبير")[currentAspectRatio]}",Toast.LENGTH_SHORT).show() } catch (e: Exception) { e.printStackTrace() } }
    private fun toggleFullscreen() { try { if(playerView.layoutParams.height==420){playerView.layoutParams.height=ViewGroup.LayoutParams.MATCH_PARENT;Toast.makeText(this,"⛶ ملء الشاشة",Toast.LENGTH_SHORT).show()}else{playerView.layoutParams.height=420;Toast.makeText(this,"📱 وضع عادي",Toast.LENGTH_SHORT).show()};playerView.requestLayout() } catch (e: Exception) { e.printStackTrace() } }
    private fun showQualityDialog() { try { AlertDialog.Builder(this).setTitle("🎯 جودة الفيديو").setItems(arrayOf("Auto","4K","1080p","720p","480p")){_,_->Toast.makeText(this,"⚙️ تلقائي",Toast.LENGTH_SHORT).show()}.show() } catch (e: Exception) { e.printStackTrace() } }
    private fun downloadCurrentStream() { try { currentStreamUrl?.let{url->try{downloadId=downloadManager.enqueue(DownloadManager.Request(Uri.parse(url)).setTitle("MN-DAZOU IPTV").setDescription(currentStreamName?:"فيديو").setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED).setDestinationInExternalPublicDir(Environment.DIRECTORY_MOVIES,"MN-DAZOU_${System.currentTimeMillis()}.mp4"));Toast.makeText(this,"⬇️ جاري التحميل",Toast.LENGTH_SHORT).show()}catch(e:Exception){Toast.makeText(this,"❌ فشل",Toast.LENGTH_SHORT).show()}}?:Toast.makeText(this,"لا يوجد فيديو",Toast.LENGTH_SHORT).show() } catch (e: Exception) { e.printStackTrace() } }
    private fun registerDownloadReceiver() { try { registerReceiver(object:BroadcastReceiver(){override fun onReceive(c:Context?,i:Intent?){if(i?.getLongExtra(DownloadManager.EXTRA_DOWNLOAD_ID,-1)==downloadId)Toast.makeText(this@MainActivity,"✅ تم التحميل",Toast.LENGTH_SHORT).show()}},IntentFilter(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) } catch (e: Exception) { e.printStackTrace() } }

    private fun playNextChannel() { try { val channels=when(currentStreamType){"live"->liveChannels;"movie"->vodMovies;else->emptyList()}; if(channels.isEmpty()||currentStreamIndex<0)return; playChannelAtIndex((currentStreamIndex+1)%channels.size) } catch (e: Exception) { e.printStackTrace() } }
    private fun playPreviousChannel() { try { val channels=when(currentStreamType){"live"->liveChannels;"movie"->vodMovies;else->emptyList()}; if(channels.isEmpty()||currentStreamIndex<0)return; playChannelAtIndex(if(currentStreamIndex-1<0)channels.size-1 else currentStreamIndex-1) } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun playChannelAtIndex(index: Int) {
        try {
            ...
            e.printStackTrace()
            Toast.makeText(this, "خطأ في التشغيل: ${e.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun showSettingsDialog() { try { AlertDialog.Builder(this).setTitle("⚙️ الإعدادات").setItems(arrayOf("🎨 تغيير الثيم","🔗 إعدادات Xtream","📋 إضافة M3U","📺 تحديث EPG","📊 إحصائيات","🗑️ مسح البيانات")){_,w->when(w){0->showThemeDialog();1->showLoginDialog();2->showM3uDialog();3->loadEpg();4->showWatchStats();5->{prefs.edit().clear().apply();accounts.clear();Toast.makeText(this,"تم المسح",Toast.LENGTH_SHORT).show();recreate()}}}.show() } catch (e: Exception) { e.printStackTrace() } }
    private fun showThemeDialog() { try { AlertDialog.Builder(this).setTitle("🎨 اختر الثيم").setItems(themes.values.map{themeItem->themeItem.name}.toTypedArray()){_,w->prefs.edit().putString("theme",themes.keys.toList()[w]).apply();Toast.makeText(this,"🔄 أعد التشغيل",Toast.LENGTH_LONG).show()}.show() } catch (e: Exception) { e.printStackTrace() } }
    private fun showM3uDialog() { try { val e=EditText(this).apply{hint="رابط M3U أو الصق المحتوى";minLines=3;setPadding(30,20,30,20)}; AlertDialog.Builder(this).setTitle("📋 إضافة M3U").setView(e).setPositiveButton("تحميل"){_,_->val i=e.text.toString(); if(i.startsWith("http"))loadM3uFromUrl(i)else parseM3uContent(i)}.setNegativeButton("إلغاء",null).show() } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun parseM3uContent(content: String) { try { val ch=mutableListOf<XtreamChannel>(); var nm=""; for(l in content.split("\n")){if(l.startsWith("#EXTINF"))nm=Regex(",(.+)").find(l)?.groupValues?.get(1)?.trim()?:"قناة"; else if(l.startsWith("http"))ch.add(XtreamChannel(ch.size,nm,"live","","","","m3u","ts"))}; liveChannels.addAll(ch); Toast.makeText(this,"✅ ${ch.size} قناة",Toast.LENGTH_SHORT).show(); updateLiveList() } catch (e: Exception) { e.printStackTrace() } }

    private fun loadEpg() { try { server?.let{srv->showLoading(); thread{try{val j=java.net.URL("${srv.url}/player_api.php?username=${srv.username}&password=${srv.password}&action=get_short_epg").readText();epgData.clear();val a=JSONObject(j).getJSONArray("epg_listings");for(i in 0 until a.length()){val o=a.getJSONObject(i);epgData.add(EpgProgram(o.optString("epg_id",""),o.optString("title",""),o.optString("start",""),o.optString("end",""),o.optString("description","")))};runOnUiThread{hideLoading();Toast.makeText(this,"📺 ${epgData.size} برنامج",Toast.LENGTH_SHORT).show();showEpg()}}catch(ex:Exception){runOnUiThread{hideLoading();Toast.makeText(this,"❌ فشل EPG",Toast.LENGTH_SHORT).show()}}}}?:Toast.makeText(this,"سجل دخول",Toast.LENGTH_SHORT).show() } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun addToFavorites(type:String,id:Int,name:String,icon:String=""){ try { if(favorites.none{fav->fav.type==type&&fav.id==id}){favorites.add(FavoriteItem(type,id,name,icon));saveFavorites();Toast.makeText(this,"⭐ تم",Toast.LENGTH_SHORT).show()}else Toast.makeText(this,"موجود",Toast.LENGTH_SHORT).show() } catch (e: Exception) { e.printStackTrace() } }
    private fun removeFavorite(item:FavoriteItem){ try { favorites.removeAll{fav->fav.type==item.type&&fav.id==item.id};saveFavorites() } catch (e: Exception) { e.printStackTrace() } }
    private fun saveFavorites(){ try { val j=JSONArray();favorites.forEach{fav->val o=JSONObject();o.put("type",fav.type);o.put("id",fav.id);o.put("name",fav.name);o.put("icon",fav.icon);j.put(o)};prefs.edit().putString("favorites",j.toString()).apply() } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun addToHistory(type:String,id:Int,name:String,icon:String=""){ try { watchHistory.removeAll{hist->hist.type==type&&hist.id==id};watchHistory.add(HistoryItem(type,id,name,System.currentTimeMillis(),icon));if(watchHistory.size>20)watchHistory.removeAt(0);saveHistory() } catch (e: Exception) { e.printStackTrace() } }
    private fun saveHistory(){ try { val j=JSONArray();watchHistory.forEach{hist->val o=JSONObject();o.put("type",hist.type);o.put("id",hist.id);o.put("name",hist.name);o.put("timestamp",hist.timestamp);o.put("icon",hist.icon);j.put(o)};prefs.edit().putString("history",j.toString()).apply() } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun playFavoriteItem(fav:FavoriteItem){ try { when(fav.type){"live"->{val u=XtreamAPI.getStreamUrl(server!!,fav.id);playStream(u,fav.name);addToHistory("live",fav.id,fav.name,fav.icon)};"movie"->{val u=XtreamAPI.getMovieUrl(server!!,fav.id);playStream(u,fav.name);addToHistory("movie",fav.id,fav.name,fav.icon)}} } catch (e: Exception) { e.printStackTrace() } }
    private fun playHistoryItem(item:HistoryItem){ try { when(item.type){"live"->playStream(XtreamAPI.getStreamUrl(server!!,item.id),item.name);"movie"->playStream(XtreamAPI.getMovieUrl(server!!,item.id),item.name)} } catch (e: Exception) { e.printStackTrace() } }

    private fun loadLiveCategories(){ try { server?.let{srv->showLoading();XtreamAPI.getLiveCategories(srv){liveCategories.clear();liveCategories.addAll(it);hideLoading();if(it.isEmpty())loadLiveStreams(null)else showLiveCategories()}}?:showLoginDialog() } catch (e: Exception) { e.printStackTrace() } }
    private fun loadVodCategories(){ try { server?.let{srv->showLoading();XtreamAPI.getVodCategories(srv){vodCategories.clear();vodCategories.addAll(it);hideLoading();if(it.isEmpty())loadMovies(null)else showVodCategories()}}?:showLoginDialog() } catch (e: Exception) { e.printStackTrace() } }
    private fun loadSeriesCategories(){ try { server?.let{srv->showLoading();XtreamAPI.getLiveCategories(srv){seriesCategories.clear();seriesCategories.addAll(it);hideLoading();if(it.isEmpty())loadSeriesList(null)else showSeriesCategories()}}?:showLoginDialog() } catch (e: Exception) { e.printStackTrace() } }

    private fun showLiveCategories(){ try { val t=themes[currentTheme]!!;tvTitle.text="📺 المجموعات (${liveCategories.size})";rv.adapter=createCategoryAdapter(liveCategories,"📁",t.accent){cat->selectedCategoryId=cat.categoryId;isShowingCategories=false;btnBack.visibility=View.VISIBLE;tvTitle.text="📺 ${cat.categoryName}";loadLiveStreams(cat.categoryId)} } catch (e: Exception) { e.printStackTrace() } }
    private fun showVodCategories(){ try { val t=themes[currentTheme]!!;tvTitle.text="🎬 المجموعات (${vodCategories.size})";rv.adapter=createCategoryAdapter(vodCategories,"🎬",t.accent){cat->selectedCategoryId=cat.categoryId;isShowingCategories=false;btnBack.visibility=View.VISIBLE;tvTitle.text="🎬 ${cat.categoryName}";loadMovies(cat.categoryId)} } catch (e: Exception) { e.printStackTrace() } }
    private fun showSeriesCategories(){ try { val t=themes[currentTheme]!!;tvTitle.text="🎭 المجموعات (${seriesCategories.size})";rv.adapter=createCategoryAdapter(seriesCategories,"📺",t.accent){cat->selectedCategoryId=cat.categoryId;isShowingCategories=false;btnBack.visibility=View.VISIBLE;tvTitle.text="🎭 ${cat.categoryName}";loadSeriesList(cat.categoryId)} } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun loadLiveStreams(catId:String?){ try { server?.let{srv->showLoading();XtreamAPI.getLiveStreams(srv,catId){hideLoading();liveChannels.clear();liveChannels.addAll(it);currentStreamIndex=-1;updateLiveList()}} } catch (e: Exception) { e.printStackTrace() } }
    private fun loadMovies(catId:String?){ try { server?.let{srv->showLoading();XtreamAPI.getVodStreams(srv,catId){hideLoading();vodMovies.clear();vodMovies.addAll(it);currentStreamIndex=-1;updateMoviesList()}} } catch (e: Exception) { e.printStackTrace() } }
    private fun loadSeriesList(catId:String?){ try { server?.let{srv->showLoading();XtreamAPI.getSeries(srv,catId){hideLoading();seriesList.clear();seriesList.addAll(it);updateSeriesList()}} } catch (e: Exception) { e.printStackTrace() } }
    ...
    private fun showEpisodesDialog(name:String,episodes:List<XtreamEpisode>){ try { AlertDialog.Builder(this).setTitle(name).setItems(episodes.map{ep->"🎭 حلقة ${ep.episodeNum}: ${ep.title}"}.toTypedArray()){_,i->val e=episodes[i];playStream(XtreamAPI.getSeriesEpisodeUrl(server!!,e.id,e.containerExtension),"$name - حلقة ${e.episodeNum}")}.setNegativeButton("إغلاق",null).show() } catch (e: Exception) { e.printStackTrace() } }
    ...
    override fun onDestroy(){ try { if(isRecording) stopRecording(); super.onDestroy(); player.release() } catch (e: Exception) { e.printStackTrace() } }
}