...
class MainActivity : AppCompatActivity() {
    ...
    private fun showHomeScreen() {
        ...
        rv.adapter=object:RecyclerView.Adapter<RecyclerView.ViewHolder>(){
            ...
            override fun onBindViewHolder(h:RecyclerView.ViewHolder,p:Int){
                ...
                else if(item is HistoryItem){(h.itemView as LinearLayout).let{l->(l.getChildAt(1) as TextView).text="🕐 ${item.name}";l.setOnClickListener{playHistoryItem(item)};if(item.icon.isNotEmpty())loadImage(item.icon){b->(l.getChildAt(0) as ImageView).setImageBitmap(b?:Bitmap.createBitmap(50,50,Bitmap.Config.ARGB_8888))}}}
                else if(item is FavoriteItem){(h.itemView as LinearLayout).let{l->(l.getChildAt(1) as TextView).text="⭐ ${item.name}";l.setOnClickListener{playFavoriteItem(item)};if(item.icon.isNotEmpty())loadImage(item.icon){b->(l.getChildAt(0) as ImageView).setImageBitmap(b?:Bitmap.createBitmap(50,50,Bitmap.Config.ARGB_8888))}}}
            }
    ...
    private fun createCategoryAdapter(cats:List<XtreamCategory>,icon:String,color:Int,onClick:(XtreamCategory)->Unit):RecyclerView.Adapter<RecyclerView.ViewHolder>{val t=themes[currentTheme]!!;return object:RecyclerView.Adapter<RecyclerView.ViewHolder>(){override fun onCreateViewHolder(p:ViewGroup,vt:Int):RecyclerView.ViewHolder{val l=LinearLayout(p.context).apply{orientation=LinearLayout.HORIZONTAL;setPadding(30,25,30,25);gravity=Gravity.CENTER_VERTICAL;setBackgroundColor(t.card)};l.addView(TextView(p.context).apply{text=icon;textSize=24f});l.addView(TextView(p.context).apply{setPadding(20,0,0,0);textSize=16f;setTextColor(t.textWhite);setTypeface(null,Typeface.BOLD);layoutParams=LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f)});l.addView(TextView(p.context).apply{text="→";textSize=20f;setTextColor(color)});return object:RecyclerView.ViewHolder(l){}}
        override fun onBindViewHolder(h:RecyclerView.ViewHolder,p:Int){val l=(h.itemView as LinearLayout);(l.getChildAt(1) as TextView).text=cats[p].categoryName;l.setOnClickListener{onClick(cats[p])}}
    ...
    private fun createContentAdapter(items:List<Pair<String,String>>,icon:String,theme:ThemeColors,onClick:(String,String)->Unit):RecyclerView.Adapter<RecyclerView.ViewHolder>{
        return object:RecyclerView.Adapter<RecyclerView.ViewHolder>(){
            override fun onCreateViewHolder(p:ViewGroup,vt:Int):RecyclerView.ViewHolder{
                val l=LinearLayout(p.context).apply{orientation=LinearLayout.HORIZONTAL;setPadding(15,12,15,12);gravity=Gravity.CENTER_VERTICAL;setBackgroundColor(theme.card)}
