...
class PlayerControlsController(
    ...
    var isFullscreen: Boolean = false
        set(value) {
            field = value
            if (!value) {
                // ✅ نخفي شريط تصفح القنوات المصغّر مباشرة بدون ما نطلب فوكس
                // على زر التشغيل — لأن شريط التحكم كله (يلي هو أصلاً حاوي
                // لهالشريط) رح ينخفي بالكامل خلال نفس اللحظة (وضع غير ملء
                // الشاشة)، فطلب الفوكس هون كان بيسبب "فوكس ياخد مكانه للحظة
                // وبعدين يختفي فجأة" — وهاد بالضبط كان يفعّل بحث أندرويد
                // الطارئ عن فوكس بديل ويتجاوز أي حجب مؤقت حطيناه.
